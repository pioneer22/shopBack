    ...
    request(options, async (error, response, body) => {
      if (error) {
        ...
        if (check_openid.length === 0) {
          ...
          let sql = "insert into user (nick_name,avatarUrl,user_sex,openid,create_time,update_time,token) values ('" + p_data.nickName + "','" + p_data.avatarUrl + "'," + p_data.gender + ",'" + p_data.openId + "','" + time + "','"+ time + "','" + token + "')"
          await db(sql)
        } else {
          // 存在时更新token
