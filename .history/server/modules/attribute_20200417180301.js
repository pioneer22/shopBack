...
router.get('/search', async (req, res) => {
  let id = parseInt(req.query.attr_id)
  ...
  let resultData = dealData.dealReturnData(result[0])
  res.send(resultData)
})
...
router.put('/edit', async (req, res) => {
  ...
  let attr_name = req.body.attr_name
  let attr_sel = req.body.attr_sel
  let attr_vals = req.body.attr_vals

  let sql = "update sp_attribute set attr_name='" + attr_name + "',attr_sel='" + attr_sel + "',attr_vals='" + attr_vals + "' where attr_id=" + id
  let result = await db(sql)
  let returnData = {}
  console.log("qqq:",result.affectedRows)

  if (result.affectedRows === 1) {
    let sql1 = "select attr_id,attr_name,cat_id,attr_sel,attr_write,attr_vals from sp_attribute where attr_id=" + id
    let result1 = await db(sql1)
    resultData = dealData.dealReturnData(result1[0])
  } else {
    resultData = dealData.constant('b')
  }
  res.send(returnData)
})
