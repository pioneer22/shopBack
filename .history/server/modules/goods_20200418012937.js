...
router.post('/addGoods', async (req, res) => {
  ...
  let attrs = req.body.attrs ? req.body.attrs : []
  let time = new Date().getTime() / 1000

  let sql = "insert into sp_goods (goods_name,goods_price,goods_number,goods_weight,cat_id,goods_introduce,add_time,upd_time,cat_one_id,cat_two_id,cat_three_id) values ('" + goods_name + "','" + goods_price + "','" + goods_number + "','" + goods_weight + "','" + goods_cat[2] + "','" + goods_introduce + "','" + time + "','" + time + "','" + goods_cat[0] + "','" + goods_cat[1] + "','" + goods_cat[2] + "')"
  ...
  result3.pics = result4
  result3.attrs = result5

  let returnData = dealData.dealReturnData(result3)
  res.send(returnData) */
})
