...
router.post('/addGoods', async (req, res) => {
 /*  let goods_name = req.body.goods_name
  ...
  /* let sql = "insert into sp_goods (goods_name,goods_price,goods_number,goods_weight,cat_id,goods_introduce,add_time,upd_time,cat_one_id,cat_two_id,cat_three_id) values ('" +
    goods_name + "','" + goods_price + "','" + goods_number + "','" + goods_weight + "','" + goods_cat[2] + "','" + goods_introduce + "','" + time + "','" + time + "','" + goods_cat[0] + "','" + goods_cat[1] + "','" + goods_cat[2] + "');select last_insert_id() as num" */

  let sql = "insert into aaaaa (name) values ('" + 666666666 + "');select last_insert_id() as num"
  let result = await db(sql)
  console.log(result)
